...
bool Player::movePlayer(char direction, Map newMap, int score) {
	vector<int> playerLocation = newMap.getPlayerLocation();
	...
	switch(direction){
		case 'a':
		{
			...
			if (playerLocation[1] - 1 < 0) { //If off bounds left side
				...
				if (checkMove(playerLocation, dungeonMap) == true) { return true; }
				if (checkTrap(playerLocation, dungeonMap) == true) { return false; }
	...
	bool move = movePlayer(_getch(), newMap, score);
	if (move == true) {
		return true;
	}
	else if (move == false) {
...
bool Player::checkMove(vector<int> playerLocation, vector< vector<char> > dungeonMap) {
...
bool Player::checkTrap(vector<int> playerLocation, vector< vector<char> > dungeonMap) {
	switch (dungeonMap[playerLocation[0]][playerLocation[1]]) {
		case 'T':
		{
			return true;
